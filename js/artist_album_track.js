...
const __ALBUM__         = 'album.tracks.get?album_id=';
const __PAGE__          = '&page=1&page_size=10';
const __API_KEY__       = '&apikey=00db4374259d474f1e530e7cf75bc23c';

// Lyrics
fetch(__BASE_URL__ + __ALBUM__ + albumId + __PAGE__ + __API_KEY__)
    .then((resp) => resp.json())
...
function showAlbum(number, ranking=1) {
    ...
    loadAlbumElement(list_element, number, ranking, button_back);
}

function loadAlbumElement(list_element, number, ranking, button_back) {
    ...
    button_detail.innerHTML = 'View Lyrics >>';

    button_detail.onclick = () => {
        location.href = './artist_album_track_lyric.html?track_id=' + track.track.track_id;
    };
    ...
    if(track.track.explicit == 0){
        explicit.innerHTML = "Explicit Word : No ";
        explicit.className += 'explicit';

    } else {
        explicit.innerHTML = "Explicit : Yes";
        explicit.className += 'explicit';
    }
    ...
    button.innerHTML = "<< View Other Album from '" + track.track.artist_name + "'";
    button.style.marginBottom = '10px';

    button.onclick = () => {
        location.href = './artist_album.html?artist_id=' + track.track.artist_id;
    };
